    ...
    <tbody>
        ...
        @foreach ($taxBrackets as $key=>$bracket)
            <tr>
                ...
                <td>
                    ${{ number_format($bracket["baseTax"]) }} plus ${{ $bracket["perDollarTax"] }} for each $1 over ${{ number_format($bracket["minThreshold"] - 1) }}
                    {{ array_key_exists($key+1, $taxBrackets) ? "and under $".number_format($taxBrackets[$key+1]["minThreshold"] - 1) : "" }}
                </td>
            </tr>
        @endforeach
    </tbody>
